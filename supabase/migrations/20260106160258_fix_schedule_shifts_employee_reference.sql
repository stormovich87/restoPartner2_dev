/*
  # Fix schedule_shifts to reference employees table

  1. Changes
    - Drop foreign key constraint from staff_members on schedule_shifts table
    - Add foreign key constraint to employees table
    - This allows schedule_shifts to work with the employees HR system
...
-- Drop the old foreign key constraint to staff_members on schedule_shifts
DO $$
BEGIN
  IF EXISTS (
...
-- Also fix schedule_rows if it exists
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
