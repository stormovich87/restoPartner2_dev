/*
  # Fix Polling Cron - Per-Courier Check

  1. Changes
    - Change logic to check per-courier instead of per-partner
    - Only skip sending if that specific courier already received poll today
    - This allows partial sends and handles courier selection changes

  2. How it works
    - Checks each courier individually
    - Sends poll only if courier hasn't received one today
    - Handles cases where courier list changes after initial send
*/
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      -- Check if this specific courier already received poll today
      SELECT EXISTS(
        SELECT 1 
      ...
      -- Skip if already sent to this courier today
