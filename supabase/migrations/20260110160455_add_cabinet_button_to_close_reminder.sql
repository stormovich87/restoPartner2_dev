/*
  # Add Cabinet Button to Close Reminder

  1. Change
    - Add "Открыть кабинет" button to shift close reminder message
    - Button opens employee cabinet via web_app (Telegram Mini App)
    - URL format: https://restopresto.org/employee/{employee_slug}

  2. Implementation
    - Update send_shift_reminders() function
    - Change close reminder to pass cabinet_url instead of NULL
    - Line 354: send_telegram_shift_reminder(..., NULL) -> send_telegram_shift_reminder(..., cabinet_url)
*/

-- Update send_shift_reminders() to add cabinet button to close reminder
  ...
  LOOP
    ...
    IF partner_record.shift_close_reminder_enabled THEN
      ...
      LOOP
        ...
        -- Build cabinet URL for close reminder
        IF shift_record.employee_cabinet_slug IS NOT NULL AND shift_record.employee_cabinet_slug != '' THEN
          cabinet_url := base_url || '/employee/' || shift_record.employee_cabinet_slug;
        ELSE
          cabinet_url := NULL;
        ...
        IF shift_record.close_reminder_sent_at IS NULL
          ...
          sent_message_id := send_telegram_shift_reminder(
            ...
            shift_record.employee_telegram_id,
            message_text,
            cabinet_url  -- Changed from NULL back to cabinet_url to add button
          );
...
'Sends shift reminders to employees via Telegram. Handles: before-shift reminders (with button), late reminders (with button), close reminders (WITH button via web_app), and auto-close. Uses employee_bot_token from partner_settings. Called every minute by cron job.';
