/*
  # Remove Cabinet Button from Close Reminder

  1. Change
    - Remove "Открыть кабинет" button from shift close reminder message
    - Keep the button in other reminder types (before-shift and late reminders)

  2. Implementation
    - Update send_shift_reminders() function
    - Change close reminder to pass NULL instead of cabinet_url
    - Line 354: send_telegram_shift_reminder(..., cabinet_url) -> send_telegram_shift_reminder(..., NULL)
*/

-- Update send_shift_reminders() to remove cabinet button from close reminder
  ...
  LOOP
    ...
    IF partner_record.shift_close_reminder_enabled THEN
      ...
      LOOP
        ...
        -- Build cabinet URL (not used for close reminder, kept for consistency)
        IF shift_record.employee_cabinet_slug IS NOT NULL AND shift_record.employee_cabinet_slug != '' THEN
          cabinet_url := base_url || '/employee/' || shift_record.employee_cabinet_slug;
        ELSE
          cabinet_url := NULL;
        ...
        IF shift_record.close_reminder_sent_at IS NULL
          ...
          -- Send message WITHOUT cabinet link button (NULL instead of cabinet_url)
          sent_message_id := send_telegram_shift_reminder(
            partner_record.employee_bot_token,
            shift_record.employee_telegram_id,
            message_text,
            NULL  -- Changed from cabinet_url to NULL to remove button
          );
...
'Sends shift reminders to employees via Telegram. Handles: before-shift reminders (with button), late reminders (with button), close reminders (NO button), and auto-close. Uses employee_bot_token from partner_settings. Called every minute by cron job.';
