/*
  # Fix Shift Reminder - Use web_app Instead of url
  
  1. Problem
    - The `send_telegram_shift_reminder()` function uses `url` button type
    - Should use `web_app` button type for Mini App integration
  
  2. Solution
    - Update function to use `web_app: { url: ... }` instead of `url: ...`
    - This matches the behavior in employee-registration-bot
  
  3. Changes
    - Line 59: Change from `'url', p_keyboard_url` to `'web_app', jsonb_build_object('url', p_keyboard_url)`
*/
