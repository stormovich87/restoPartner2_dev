/*
  # Fix External Courier Polling Cron - Direct Approach

  1. Changes
    - Replace HTTP call approach with direct database operations
    - Create function that directly handles poll sending logic
    - Uses pg_net to call Telegram API directly
    - No need for app.supabase_url or service_role_key settings

  2. How it works
    - Cron runs every minute
    - Function checks schedules and sends Telegram messages directly
    - Creates response records in database
*/

-- Create function to send Telegram message
...
BEGIN
  ...
  SELECT INTO response_data content::jsonb
  FROM net.http_post(
  ...
  IF response_data IS NOT NULL AND response_data->'result'->'message_id' IS NOT NULL THEN
...
-- Update main function to send polls directly
  ...
  LOOP
    ...
    LOOP
      ...
      IF sent_message_id IS NOT NULL THEN
        ...
        ON CONFLICT (partner_id, courier_id, response_date) DO NOTHING;
      END IF;
    END LOOP;
