/*
  # Improve shift reminder message text formatting

  1. Changes
    - Update message text in `send_shift_reminders()` to use clean HTML formatting
    - Add emojis for better visual perception
    - Remove raw \n characters, use proper HTML structure
    
  2. Message formats
    - Before-shift: "⏰ Напоминание о смене" with branch and start time
    - Late reminder: "⚠️ Смена не открыта" with branch and start time
    - Comment displayed in italic at the end if provided
*/
  ...
  LOOP
    ...
    LOOP
      ...
      IF shift_record.reminder_before_sent_at IS NULL 
        ...
        -- Build message with clean HTML formatting and emojis
        message_text := '⏰ <b>Напоминание о смене</b>

🏬 <b>Филиал:</b> ' || branch_name || '
🕒 <b>Начало:</b> ' || TO_CHAR(shift_record.start_time, 'HH24:MI') || '

Пожалуйста, не забудьте открыть смену в кабинете.';

        IF partner_record.shift_reminder_comment IS NOT NULL AND partner_record.shift_reminder_comment != '' THEN
          message_text := message_text || '

<i>' || partner_record.shift_reminder_comment || '</i>';
        END IF;
      ...
      IF shift_record.reminder_late_sent_at IS NULL 
        ...
        -- Build message with clean HTML formatting and emojis
        message_text := '⚠️ <b>Смена не открыта</b>

🏬 <b>Филиал:</b> ' || branch_name || '
🕒 <b>Начало:</b> ' || TO_CHAR(shift_record.start_time, 'HH24:MI') || '

Пожалуйста, откройте смену как можно скорее.';
