...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE pubname = 'supabase_realtime' AND tablename = 'kpi_indicator_catalog'
  ) THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE kpi_indicator_catalog;
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime' AND tablename = 'kpi_section_rank_weights'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime' AND tablename = 'kpi_template_indicators'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime' AND tablename = 'kpi_indicator_rank_settings'
  ) THEN
